...
export const resetTokenEmail: EmailTemplateGenerator = function(
  ...
  const isVmt = appName === AppNames.Vmt;
  const authEndpoint = isVmt ? 'resetPassword' : '#/auth/reset';
...
export const confirmEmailAddress: EmailTemplateGenerator = function(
  ...
  const isVmt = appName === AppNames.Vmt;
  const authEndpoint = isVmt ? 'confirmEmail' : '#/auth/confirm';
...
export const newUserNotification: EmailTemplateGenerator = function(
  ...
  const { username } = user;
  const isVmt = appName === AppNames.Vmt;
  const appNameModifier = isVmt ? 'a' : 'an';
  const subject = `A new user has registered for ${appNameModifier} ${appName} account`;
  const isGoogleUser = typeof user.googleId === 'string';
  const registerDesc = isGoogleUser ? 'signed up via Google' : 'signed up';

  const msg = isVmt
    ? `A new user (username: ${username}) has ${registerDesc} for a ${appName} account and is now authorized. Please visit ${host}/ for further review.`
    : `A new user (username: ${username}) has ${registerDesc} for an ${appName} account and is waiting to be authorized. Please visit ${host}/ to login and navigate to the users portal to view users that are waiting for authorization.`;
...
export const googleSignup: EmailTemplateGenerator = function(
  ...
  return {
    ...
    text: `Congratulations! You have successfully signed up for ${appNameModifier} ${appName} account through your Google account (${user.email}). This email is the username of your new account. Please visit ${host}/ to login and begin exploring the software.
